...
export class AuthService {
  ...
  registerUser(user: { email: string; password: string }) {
    ...
    this.http
      .post<{ token: string; payloadEmail: string }>(
        environment._registerURL,
        user
      )
  ...
  deleteAccount() {
    this.http
      .delete<{ statusCode: String }>(
        environment._deleteAccountUrl + this.getEmail()
      )
      .subscribe(
        (res) => {
  ...
  verifyToken() {
    return this.http.get<any>(environment._verification);
  }
